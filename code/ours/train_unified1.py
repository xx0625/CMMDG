"""
统一训练脚本 - EEG Positional Transformer
===========================================
支持三种实验配置和两种训练模式，融合了之前分散的多个训练文件。
...
EXPERIMENT_CONFIGS = {
    "4db_14ch": {
        ...
        "ppt_weight1": 0.09,
        "ppt_weight2": 0.11,
        "weight_consistency": 0.88,
        "weight_exp": 0.05,
        "weight_domain": 0.23,
        "weight_positional_matrix": 0.031,
        # 样本筛选参数 (reject-all)
        "alpha": 0.77,
        "beta": 1.02,
        "gamma": 0.8,
    ...
    "2db_56ch": {
        ...
        "weight_exp": 0.06,
        "weight_domain": 0.01,
        "weight_positional_matrix": 0.034,
        "alpha": 1.12,
        "beta": 1.06,
        "gamma": 1.02,
        "rho": 0.16,
...
def plot_loss_curves(train_losses, val_losses, test_db, save_path, suffix=""):
    ...
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    plt.title(f'Loss Curves - Test on {test_db} (Model{MODEL_VERSION}) {suffix}')
    plt.legend()
    plt.grid(alpha=0.3)
    plt.tight_layout()
    plt.savefig(os.path.join(save_path, f'loss_curve_model{MODEL_VERSION}_{test_db}{suffix}.png'), dpi=300)
    plt.close()
...
def load_and_reshape(csv_path, db_name, n_timesteps, channel, window_type=None):
    """
    加载CSV并重塑为 (samples, channel, timesteps)。
    channel=56时使用 new_indices 重排列顺序；channel=14时取前14列。
    """
...
def prepare_data(train_dbs, test_db, base_path, load_suffix, unload_suffix, n_timesteps, channel):
    ...
    # 增加通道维度: (N, channel, timesteps) -> (N, 1, channel, timesteps)
...
def get_model_class():
    ...
    return model_module.EEGPositionalTransformer
...
def standard_train_loop(model, optimizer, scheduler, early_stopping,
                        ...
                        ppt_weight1, ppt_weight2, weight_consistency, weight_exp, weight_domain,
                        test_db, save_dir):
    ...
    for epoch in tqdm(range(max_epochs), desc=f"Train (Test: {test_db})"):
        ...
        for batch_start in range(0, X_train.size(0), batch_size):
            ...
            loss_main = criterion_cls(final_pred, batch_y)
            ...
            for k in range(num_train_domains):
                mask = (batch_d == k)
                if mask.sum() > 0:
                    loss_experts += criterion_cls(expert_outs[mask, k], batch_y[mask])
                    valid_experts += 1
            if valid_experts > 0:
            ...
            loss_consistency = criterion_cls(mixture_other_logits, batch_y)

            loss_ppt_aux = ppt_weight1 * order_loss + ppt_weight2 * recon_loss

            prototypes = model.gate_generator.prototypes
            loss_domain = compute_discriminative_domain_loss(fusion_embed, batch_d, prototypes, temperature=0.1)

            total_loss = loss_main + weight_exp * loss_experts + loss_ppt_aux + \
                         (weight_consistency * loss_consistency) + (weight_domain * loss_domain)
...
def pretrain_stage1(model, optimizer, scheduler, X_train, y_train, train_domains, X_val, y_val,
    ...
    for epoch in tqdm(range(max_epochs), desc=f"Stage 1 ({test_db})", leave=False):
        ...
        for batch_start in range(0, X_train.size(0), batch_size):
            ...
            loss_ppt = ppt_weight1 * order_loss + ppt_weight2 * recon_loss
            loss_domain = compute_discriminative_domain_loss(
                fusion_embed, batch_d, model.gate_generator.prototypes, 0.1)

            total_loss = loss_main + weight_exp * loss_experts + loss_ppt + \
                         weight_consistency * loss_consistency + weight_domain * loss_domain
...
# ===================== Stage 2 样本打分筛选 (reject-all) =====================
def evaluate_and_prune_fused(model, X_train, y_train, train_domains, batch_size,
                             prune_ratio, rho, alpha, beta, gamma):
    model.eval()
    criterion_none = nn.CrossEntropyLoss(reduction='none')
    all_confidences, all_similarities, all_gaps = [], [], []
    prototypes = model.gate_generator.prototypes.detach()

    print(f"\n[*] 综合跨域一致性打分 (Alpha={alpha}, Beta={beta}, Gamma={gamma}, Rho={rho})...")
    ...
    total = len(X_train)
    print(f"[*] 剔除完成: {total} -> {kept} (剔除 {total - kept} 个低质量样本)")
...
def run_train_all(cfg, test_db, train_dbs, X_train, y_train, train_domains,
    ...
    EEGPositionalTransformer = get_model_class()
    matrix_path = os.path.join(MATRIX_DIR, cfg["matrix_file"])

    setup_seed(GLOBAL_SEED)
    model = EEGPositionalTransformer(
        ...
        weight_positional_matrix=cfg["weight_positional_matrix"]
    ).to(device)
    ...
    standard_train_loop(
        ...
        ppt_weight1=cfg["ppt_weight1"], ppt_weight2=cfg["ppt_weight2"],
        weight_consistency=cfg["weight_consistency"], weight_exp=cfg["weight_exp"],
        weight_domain=cfg["weight_domain"],
...
def run_reject_all(cfg, test_db, train_dbs, X_train, y_train, train_domains,
    ...
    if not os.path.exists(ckpt_stage1):
        setup_seed(GLOBAL_SEED)
        model_s1 = EEGPositionalTransformer(
            n_timesteps=cfg["n_timesteps"], n_electrodes=cfg["channel"],
            n_classes=2, sampling_rate=128, num_domains=num_train_domains,
            batch_size=batch_size, positional_matrix_path=matrix_path,
            patch_size=16, weak_permutation_ratio=0.2, strong_permutation_ratio=0.8,
    ...
    clean_X, clean_y, clean_d = evaluate_and_prune_fused(
        ...
        rho=cfg["rho"], alpha=cfg["alpha"], beta=cfg["beta"], gamma=cfg["gamma"]
    )
    del model_s1
    torch.cuda.empty_cache()
    ...
    setup_seed(GLOBAL_SEED)
    model_s3 = EEGPositionalTransformer(
        n_timesteps=cfg["n_timesteps"], n_electrodes=cfg["channel"],
        n_classes=2, sampling_rate=128, num_domains=num_train_domains,
        batch_size=batch_size, positional_matrix_path=matrix_path,
        patch_size=16, weak_permutation_ratio=0.2, strong_permutation_ratio=0.8,
...
def main():
    parser = argparse.ArgumentParser(description="统一 EEG 训练脚本")
    ...
    for i, test_db in enumerate(databases):
        ...
        model.eval()
        # 域相似度分析
        analyze_domain_similarity(model, X_train, y_train, train_domains,
                                  f"TRAIN Set (Test={test_db})", train_dbs, batch_size, device)
        analyze_domain_similarity(model, X_val, y_val, val_domains,
                                  f"VALIDATION Set (Test={test_db})", train_dbs, batch_size, device)
        analyze_domain_similarity(model, X_test, y_test, None,
