...
class Adapted_DG_Network(nn.Module):
    def __init__(self, classes=2, channels=14, time_steps=128, F1=8, D=2, domains=1):
...
if __name__ == "__main__":
    ...
    for i, test_db in enumerate(databases):
        ...
        # 动态传入 domains 数量
        setup_seed(42)
        model = Adapted_DG_Network(classes=2, channels=14, time_steps=128, domains=num_source_domains).to(DEVICE)
