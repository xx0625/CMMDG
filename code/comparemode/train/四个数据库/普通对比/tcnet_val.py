...
def prepare_data(train_dbs, test_db, base_path, load_suffix, unload_suffix, n_timesteps=128, channel=14):
    ...
    # TCNet input: (Batch, 1, Channels, Time)
...
if __name__ == "__main__":
    ...
    for test_db in databases:
        ...
        # 1. 第一次 setup_seed(42)：保证“考卷”（数据划分、打乱）完全一致
        setup_seed(42)
        ...
        # 2. 第二次 setup_seed(42)：保证“起跑线”（模型权重初始化）完全一致
        setup_seed(42)
        # 定义正式训练的模型
        model = EEGTCNet(n_classes=2, eeg_chans=14).to(device)
        ...
        print(f"FLOPs (G): {flops * 2 / 1e9:.4f}")
        print(f"FLOPs (M): {flops * 2 / 1e6:.4f}")
        # === 修改处：计算FLOPs 使用独立的临时模型 ===
        if not flops_calculated:
            print("\n=== 计算模型参数量和FLOPs ===")
            setup_seed(42)  # 临时模型也保持一致
            flops_model = EEGTCNet(n_classes=2, eeg_chans=14).to(device)
            dummy_input = torch.randn(1, 1, channel, n_timesteps).to(device)

            try:
                flops, params = profile(flops_model, inputs=(dummy_input,), verbose=False)
