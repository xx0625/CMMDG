...
# 保留全局默认的 seed 初始化，以防外部依赖
setup_seed(42)
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
print(f"Using device: {device}")


# 核心修改：将默认 window_type 从 'hamming' 改为 'None'
...
def prepare_data(train_dbs, test_db, base_path, load_suffix, unload_suffix, n_timesteps=256, channel=14):
    ...
    load_test, load_identities = load_and_reshape(os.path.join(base_path, f"{test_db}{load_suffix}"), test_db,
                                                  n_timesteps, channel, window_type='None')
    ...
    # TS_SEFFNet input shape: (Batch, Channel, Time, 1)
...
if __name__ == "__main__":
    ...
    for i, test_db in enumerate(databases):
        ...
        # 1. 第一次 setup_seed(42)：保证“考卷”完全一致
        setup_seed(42)
        X_train, y_train, train_identities, X_val, y_val, val_identities, X_test, y_test, test_identities = prepare_data(
            train_dbs, test_db, base_path, load_suffix, unload_suffix, n_timesteps, channel=channel
        )
        ...
        # 2. 第二次 setup_seed(42)：保证“起跑线”完全一致
        setup_seed(42)
        model = TS_SEFFNet(channel, 2).to(device)

        # === 计算FLOPs ===
        if not flops_calculated:
            print("\n=== 计算模型参数量和FLOPs ===")
            setup_seed(42)  # 临时输入也保证一致
            # TSSEFFNet input shape: (Batch, Channels, Time, 1)
            dummy_input = torch.randn(1, channel, n_timesteps, 1).to(device)
            try:
                flops, params = profile(model, inputs=(dummy_input,), verbose=False)
                print(f"总参数量: {params / 1e6:.4f} M")
                print(f"总FLOPs: {flops *2.0/ 1e9:.4f} G")
        ...
        # === 核心修改：添加 ReduceLROnPlateau 学习率调度器 ===
