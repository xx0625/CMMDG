...
from model.eegent import EEGNet
import thop
...
if __name__ == "__main__":
    ...
    for i, test_db in enumerate(databases):
        # 修改点 3: 第一次 setup_seed(42)，在 prepare_data 之前，保证“考卷”完全一致
        setup_seed(42)
        ...
        # 修改点 3: 第二次 setup_seed(42)，在 model = 之前，保证“起跑线”完全一致
        setup_seed(42)
        model = EEGNet(nb_classes=2, Chans=channel, Samples=n_timesteps).to(device)
        ...
        # 修改点 1: 使用 ReduceLROnPlateau 动态调整学习率（factor=0.5, patience=5）
        ...
        early_stopping = EarlyStopping(
            patience=patience,
            verbose=True,
            path=checkpoint_path  # 传入统一构造的路径
        )
        ...
        for epoch in tqdm(range(max_epochs), desc=f"Epochs (Test DB: {test_db})"):
            ...
            # 修改点 1: 在计算完 val_loss 后调用 scheduler 进行学习率更新
        ...
        plt.title(f'{model_name} Loss Curve - {test_db}')  # 标题也加上模型名
        plt.xlabel('Epochs')
        plt.ylabel('Loss')
        plt.legend()
        plt.grid(True)
        ...
        plt.title(f'{model_name} Accuracy Curve - {test_db}')  # 标题也加上模型名
        plt.xlabel('Epochs')
        plt.ylabel('Accuracy')
        plt.legend()
        plt.grid(True)

        # 保存图片：eegnet_curve_nback.png
        ...
        # === 测试阶段 & 结果保存 ===
        # 使用之前统一构造的 checkpoint_path 加载模型，避免路径不一致报错
