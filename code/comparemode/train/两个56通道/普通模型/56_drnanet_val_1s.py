...
def load_and_reshape(csv_path, db_name):
    ...
    # === 修改点1：步长改为1，不需要进行5的倍数截断 ===
    max_samples = len(data)
    data = data[:max_samples]
    identities = identities[:max_samples]

    # 验证数据完整性
    assert len(data) == len(identities), "数据与身份信息长度不一致!"

    # === 修改点2：重塑数据形状为 (n_groups, 1, features) ===
    data = data.reshape(-1, 1, psd_feature)

    # === 修改点3：同步重塑身份信息为 (n_groups, 1) ===
    identities = identities.reshape(-1, 1)
...
if __name__ == "__main__":
    ...
    for test_db in databases:
        ...
        if not flops_calculated:
            print("\n=== 计算模型参数量和FLOPs ===")
            # === 修改点4：测试FLOPs时的虚拟输入序列长度改为1 ===
            dummy_input = torch.randn(1, 1, psd_feature).to(device)
            try:
