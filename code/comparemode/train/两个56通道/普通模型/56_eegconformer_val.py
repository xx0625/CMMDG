...
if __name__ == "__main__":
    ...
    for i, test_db in enumerate(databases):
        ...
        # 修改点 3 (第一处): 在数据处理前固定随机种子，保证“考卷”完全一致
        setup_seed(42)
        ...
        # 修改点 3 (第二处): 在模型实例化前固定随机种子，保证“起跑线”完全一致
        setup_seed(42)
        model = EEGConformer(
            n_chans=channel,
            n_times=n_timesteps,
        ...
        if not flops_calculated:
            print("\n=== 计算模型参数量和FLOPs ===")
            # Conformer输入通常是 (Batch, Channels, Time)
            ...
            try:
                ...
                print(f"计算FLOPs出错: {e} (可能是Braindecode模型结构特殊，跳过)")
            flops_calculated = True

        optimizer = optim.Adam(model.parameters(), lr=0.001)
        criterion = nn.CrossEntropyLoss()

        # 修改点 1: 添加 ReduceLROnPlateau
        ...
        for epoch in tqdm(range(max_epochs), desc=f"Epochs (Test DB: {test_db})"):
            ...
            # 修改点 1: 每轮验证结束后让 Scheduler 监测 val_loss 并动态调整学习率
