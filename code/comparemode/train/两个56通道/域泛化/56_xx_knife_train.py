...
def prepare_data(train_dbs, test_db, base_path, load_suffix, unload_suffix, n_timesteps=128):
    ...
    for db in train_dbs:
        ...
        # LOAD
        load_data, _ = load_and_reshape(os.path.join(base_path, f"{db}{load_suffix}"), db, n_timesteps, 56)
        ...
        # UNLOAD
        unload_data, _ = load_and_reshape(os.path.join(base_path, f"{db}{unload_suffix}"), db, n_timesteps, 56)
    ...
    # Test Data
    load_test, _ = load_and_reshape(os.path.join(base_path, f"{test_db}{load_suffix}"), test_db, n_timesteps, 56)
    unload_test, _ = load_and_reshape(os.path.join(base_path, f"{test_db}{unload_suffix}"), test_db, n_timesteps, 56)
